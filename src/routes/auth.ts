...
// ===== LOGOUT-FUNKTION =====
async function performLogout(client: AxiosInstance, jar: CookieJar): Promise<void> {
  try {
...
async function scrapeData(client: AxiosInstance, cookieString: string, socketId?: string): Promise<any> {
  try {
    if (socketId && io) {
    ...
    $('table tr').each((index, element) => {
      if (index === 0) return; // Überspringe Header
      ...
      $(element).find('td').each((cellIndex, cell) => {
        ...
        switch(cellIndex) {
          case 0: row.id = text; break;
          case 1: row.datum = text; break;
          case 2: row.kategorie = text; break;
          case 3: row.mitarbeiter = text; break;
          case 4: row.name = text; break;
          case 5: row.plz = text; break;
          case 6: row.anbieter = text; break;
          case 7: row.firma = text; break;
          case 8: row.kdnr = text; break;
          case 9: row.wechsel = text; break;
          case 10: row.strom = text; break;
          case 11: row.gas = text; break;
          case 12: row.frist = text; break;
          case 13: row.aktion = text; break;
          default: break;
        }
      });
...
async function scrapeMitarbeiter(client: AxiosInstance, cookieString: string): Promise<any> {
  try {
    ...
    $('table thead tr th').each((index, element) => {
      ...
      if (headerText) {
        ...
        if (headerText.includes('führung')) fuehrungskraftIndex = index;
        if (headerText.includes('registrator')) registratorIndex = index;
        if (headerText.includes('nachname') || (headerText.includes('name') && !headerText.includes('vorname'))) nameIndex = index;
        if (headerText.includes('vorname')) vornameIndex = index;
        if (headerText.includes('aktion')) aktionIndex = index;
        if (headerText === 'gpnr') gpnrIndex = index;
        if (headerText === 'vcnr') vcnrIndex = index;
      }
    });

...
async function scrapeAllErledigteFormulare(client: AxiosInstance, cookieString: string): Promise<Map<string, number>> {
  try {
    ...
    const formulareCounts = new Map<string, number>();
    ...
    $('table tbody tr, table tr').each((index, row) => {
      ...
      if (hasHeaderCells || index === 0) return;
...
// API Route
router.post('/check-formulare', async (req: Request, res: Response) => {
...
router.post('/download-all', async (req: Request, res: Response) => {
...
router.post('/download-bk', async (req: Request, res: Response) => {
  const { socketId } = req.body;
  const GPNR = '60235';
  const PASSWORD = 'r87cucd';

...
async function scrapeOrganigramm(client: AxiosInstance, cookieString: string, socketId?: string): Promise<any> {
  try {
    if (socketId && io) {
    ...
    $('p').each((i, elem) => {
      ...
      if (direkteMatch) stats.direkte = parseInt(direkteMatch[1]);
      if (gfMatch) stats.gfGps = parseInt(gfMatch[1]);
      if (vcMatch) stats.vcGps = parseInt(vcMatch[1]);
    });
    
    console.log('[ORGANIGRAMM] Statistiken:', stats);
    
    // Rekursive Funktion um Hierarchie zu parsen
    function parseUserCard($card: any, level: number = 0, parentId: string | null = null): any {
    ...
    function traverseHierarchy($li: any, level: number = 0, parentId: string | null = null): any[] {
      const results: any[] = [];
      ...
      if ($card.length > 0) {
        ...
        if (userData) {
          ...
          if ($childrenList.length > 0) {
            $childrenList.find('> li').each((_i: number, childLi: any) => {
...
// ===== SUPABASE SPEICHERN =====
async function saveToSupabase(organigrammData: any[], socketId?: string): Promise<any> {
  try {
    if (socketId && io) {

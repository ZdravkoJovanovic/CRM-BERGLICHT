...
router.post('/scrape-mitarbeiter', async (req, res) => {
    ...
    try {
        ...
        $('table').each((tableIndex, table) => {
            ...
            if (!headers.includes('GPNr'))
                return; // Skip diese Tabelle
...
router.post('/scrape-leads', async (req, res) => {
    ...
    try {
        ...
        for (let i = 0; i < mitarbeiterMitLeads.length; i++) {
            ...
            const nameParts = [];
            if (mitarbeiter.gpnr)
                nameParts.push(mitarbeiter.gpnr);
            if (mitarbeiter.vcnr)
                nameParts.push(mitarbeiter.vcnr);
            if (mitarbeiter.cisnr)
                nameParts.push(mitarbeiter.cisnr);
...
router.post('/scrape-bk-formulare', async (req, res) => {
    const { socketId } = req.body;
    const GPNR = '60235';
    const PASSWORD = 'r87cucd';
    const jar = new tough_cookie_1.CookieJar();
                        ...
                        $detail('label').each((idx, label) => {
                            ...
                            if (labelText.includes('Geburtsdatum')) {
                                ...
                                if (input.length > 0 && !formular.geburtsdatum) {
                                    formular.geburtsdatum = input.val() || '';
                                }
                            }
                        });
                    }
...
router.post('/scrape-bk-mit-pdfs', async (req, res) => {
    const { socketId } = req.body;
    const GPNR = '60235';
    const PASSWORD = 'r87cucd';
    const PARALLEL_WORKERS = 100;
    const jar = new tough_cookie_1.CookieJar();
    const client = (0, axios_cookiejar_support_1.wrapper)(axios_1.default.create({ jar, withCredentials: true, validateStatus: () => true, maxRedirects: 5, timeout: 60000 }));
    try {
        ...
        const processFormular = async (formular) => {
            const wClient = axios_1.default.create({
            ...
            try {
                ...
                await s3Client.send(new client_s3_1.PutObjectCommand({ Bucket: S3_BUCKET_BK_MIT_PDFS, Key: `${mName}/${kName}/formular.csv`, Body: csv, ContentType: 'text/csv; charset=utf-8' }));
                ...
                $d('a').each((i, link) => {
                    ...
                    if (href.endsWith('.pdf'))
                        pdfUrls.push(href.startsWith('http') ? href : `${PROJECT_BASE_URL}/${href}`);
                });
                for (const pUrl of pdfUrls) {
                    try {
                        ...
                        if (pResp.status === 200) {
                            ...
                            await s3Client.send(new client_s3_1.PutObjectCommand({ Bucket: S3_BUCKET_BK_MIT_PDFS, Key: `${mName}/${kName}/${pName}`, Body: Buffer.from(pResp.data), ContentType: 'application/pdf' }));
        ...
        if (socketId && io)
            io.to(socketId).emit('bk-pdfs-complete', { formulare: processedFormulare, pdfs: totalPdfs, sizeMB: (totalUploadedBytes / (1024 * 1024)).toFixed(2) });
